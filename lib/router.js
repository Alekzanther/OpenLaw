...
ControversialArticlesController = ArticlesListController.extend({
  sort: {submitted: -1, _id: -1},
  nextPath: function() {
    return Router.routes.controversialArticles.path({articlesLimit: this.articlesLimit() + this.increment})
  }
});

HotArticlesController = ArticlesListController.extend({
  sort: {upvotes: -1, submitted: -1, _id: -1},
  nextPath: function() {
    return Router.routes.hotArticles.path({articlesLimit: this.articlesLimit() + this.increment})
  }
});

PopularArticlesController = ArticlesListController.extend({
  sort: {upvotes: -1, submitted: -1, _id: -1},
  nextPath: function() {
    return Router.routes.popularArticles.path({articlesLimit: this.articlesLimit() + this.increment})
  }
});

UnpopularArticlesController = ArticlesListController.extend({
  sort: {upvotes: -1, submitted: -1, _id: -1},
  nextPath: function() {
    return Router.routes.unpopularArticles.path({articlesLimit: this.articlesLimit() + this.increment})
  }
});
...
Router.route('/profile/:comment?', {name: 'profilePage'});

Router.route('/new/:articlesLimit?', {name: 'newArticles'});

Router.route('/hot/:articlesLimit?', {name: 'hotArticles'});

Router.route('/controversial/:articlesLimit?', {name: 'controversialArticles'});

Router.route('/popular/:articlesLimit?', {name: 'popularArticles'});

Router.route('/unpopular/:articlesLimit?', {name: 'unpopularArticles'});
